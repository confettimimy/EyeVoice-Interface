...
def gaze_data_callback(gaze_data):
    gaze_left_eye = gaze_data['left_gaze_point_on_display_area'] # 왼쪽 눈 데이터, gaze_left_eye = (x, y)
    gaze_right_eye = gaze_data['right_gaze_point_on_display_area'] # 오른쪽 눈 데이터, gaze_right_eye = (x, y)

    xs = (gaze_left_eye[0], gaze_right_eye[0]) # (왼쪽x값, 오른쪽x값)
    ys = (gaze_left_eye[1], gaze_right_eye[1]) # (왼쪽y값, 오른쪽y값)

    # if all of the axes have data from at least one eye
    if all([x != -1.0 for x in xs]) and all([y != -1.0 for y in ys]): # xs, ys(튜플 자료형)에 있는 요소 모두가 -1.0이 아니라면
        # take x and y averages
        gaze_data_callback = (np.nanmean(xs), np.nanmean(ys)) # nan값을 무시하고 xs, ys(튜플)의 평균값
    else:
